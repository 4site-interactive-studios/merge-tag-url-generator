...
function addMergeTag() {
  ...
  tagName.addEventListener("keydown", function () {
    clearTimeout(typingTimer);
  });
  tagName.addEventListener("keyup", function () {
    clearTimeout(typingTimer);
  ...
  var tagDataLabel = document.createElement("label");
  tagDataLabel.innerHTML = "Merge Tag Data";
  var tagData = document.createElement("textarea");
  tagData.classList.add("merge-tag-data");
  tagData.addEventListener("focusout", generateURL);
  tagData.addEventListener("keydown", function () {
    clearTimeout(typingTimer);
  });
  tagData.addEventListener("keyup", function () {
    clearTimeout(typingTimer);
    typingTimer = setTimeout(generateURL, 1000);
  });
...
function generateURL() {
  var newURLContainer = document.querySelector(".generated-url");
  var mergeTags = document.querySelectorAll(".merge-tag-info");
  var originalURLContainer = document.querySelector(".url-input");
  var newURL;
  ...
  mergeTags.forEach(function (tag) {
    var tagName = tag.querySelector(".merge-tag-name");
    var tagData = tag.querySelector(".merge-tag-data");

    if (tagName.value.length > 0 && tagData.value.length > 0) {
      var queryTagName = "engrid_data[".concat(tagName.value, "]");
      var queryTagData = tagData.value;
...
function setIframe(withTags) {
  var iframe = document.querySelector(".merge-tag-url");
  var origURL = document.querySelector(".url-input");
  var generatedURL = document.querySelector(".generated-url");
...
function copyURL() {
  var url = document.querySelector(".generated-url");
  url.select();
...
window.onload = function () {
  ...
  originalURL.addEventListener("keyup", function () {
    clearTimeout(typingTimer);
  ...
  document.querySelector("input[value='with-tags']").addEventListener("click", function (e) {
    setIframe(true);
  });
  document.querySelector("input[value='without-tags']").addEventListener("click", function (e) {
    setIframe(false);
  });
...
// Module
exports.push([module.i, ".merge-tag-info{display:flex;flex-direction:row;flex-wrap:wrap;row-gap:20px}.merge-tag-info label{flex-basis:100%}.merge-tag-container{display:flex;flex-direction:column;row-gap:20px}.generated-url{font-size:20px;font-weight:bold;width:80%;height:350px}", ""]);
// Exports
